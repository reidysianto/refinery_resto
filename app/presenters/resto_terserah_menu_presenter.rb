class RestoTerserahMenuPresenter
  ...
  def render_menu_items(menu_items)
    if menu_items.present?
      content_tag(list_tag, :class=> @css_list_tag) do
        @css_list_tag="dropdown-menu"
  ...
  def render_menu_item(menu_item, index)
    content_tag(list_item_tag, :class => menu_item_css(menu_item, index)) do
      ...
      if menu_item.has_children? && index != 0 then
        buffer <<link_to( menu_item.title ,context.refinery.url_for(menu_item.url), class: "dropdown-toggle " )
      else
  ...
  def menu_item_css(menu_item, index)
    ...
    css << "dropdown" if menu_item.has_children?
